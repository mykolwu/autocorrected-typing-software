...
def accuracy(typed, source):
    ...
    if len(typed_words) == len(source_words) and len(source_words) == 0:
        return 100.0
    word_count = min(len(typed_words), len(source_words))
    if word_count == 0:
        return 0.0
    else:
        correct_words = 0
    ...
    # One line version:
    return (100 * sum([a == b for a, b in zip(typed_words, source_words)]) / len(typed_words)) if len(typed_words) > 0 else 100.0*(len(typed_words) == len(source_words))
...
def wpm(typed, elapsed):
    ...
    typed_pseudo_length = len(typed) / 5
    duration = elapsed / 60
    return typed_pseudo_length / duration
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    diffs = {w: diff_function(typed_word, w, limit) for w in word_list}
    if min(diffs.values()) > limit:
        return typed_word
    return min(word_list, key=diffs.get)

    # Alternative implementation (no dictionary)
    if typed_word in word_list:
        return typed_word
    word = min(word_list, key=lambda w: diff_function(typed_word, w, limit))
...
def time_per_word(words, times_per_player):
    ...
    [[6, 3, 6, 2], [10, 6, 1, 2]]
    """
    n_words, n_players = range(len(words)), range(len(times_per_player))
    tpp = times_per_player
    times = [[tpp[j][i+1] - tpp[j][i] for i in n_words] for j in n_players]
...
def fastest_words(match):
    ...
    fastest = [min(player_indices, key=lambda p: time(match, p, i)) for i in word_indices]
    return [[get_word(match, i) for i in word_indices if fastest[i] == p] for p in player_indices]
